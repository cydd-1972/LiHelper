...
@tool(
    description=(
        "从向量存储检索综测政策原文。回答涉及分值时须结合模块/基础分发展分/占比说明，"
        "不可把表格数字说成综测总分直接加这么多。"
    )
)
def rag_summarize(
...
@tool(description="使用高级检索策略检索综测政策，MQE多查询扩展提高召回率")
def rag_search_mqe(query: str) -> str:
    """使用多查询扩展(MQE)策略检索综测相关内容"""
    return rag.rag_summarize(query, strategy="mqe")


@tool(description="使用假设文档嵌入(HyDE)策略检索综测政策，适合复杂问题")
def rag_search_hyde(query: str) -> str:
    """使用假设文档嵌入(HyDE)策略检索综测相关内容"""
    return rag.rag_summarize(query, strategy="hyde")


@tool(description="使用扩展检索框架检索综测政策，同时启用MQE和HyDE，效果最好")
def rag_search_expanded(query: str) -> str:
    """使用扩展检索框架，综合MQE和HyDE检索综测内容"""
...
@tool(description="检索指定学生在指定学期的完整综测记录，以纯字符形式返回")
def fetch_student_data(user_id: str, semester: str) -> str:
    ...
    try:
        ...
        result = f"学号{user_id}在{semester}学期的综测记录：\n"
        ...
        result += f"综测总分：{data['总分']}分\n"
        result += f"年级排名：{data['年级排名']}"
        return result
    except KeyError:
        logger.warning(f"[fetch_student_data]未能检索到学生:{user_id}在{semester}的数据。")
        return f"未找到学号{user_id}在{semester}学期的综测记录"


@tool(description="计算综测分数，输入各项加分明细，返回计算结果")
def calculate_score(
    ...
    result += f"综测总分：{total_score:.2f}分"
    
    return result


@tool(description="无入参，无返回值，调用后触发中间件自动为报告生成场景动态注入上下文信息")
